...
K 线图，也叫蜡烛图，用一根柱子展示一个周期内的开盘价、最高价、最低价和收盘价。

在本项目的 Streamlit 页面中，K 线支持：
...
日线适合观察短期走势，周线和月线更适合中长期趋势分析。用户提到“不需要频次太高”，所以当前没有加入分钟线。
...
- `bullish`：`trend_score >= 0.55`，偏多趋势
- `bearish`：`trend_score <= -0.55`，偏空趋势
- `neutral`：介于两者之间，趋势不够明确
...
当前打分从 50 分开始，然后按以下方向加减分：

```text
score = 50
      + trend_score * 14
      + clip(momentum_20 * 100, -16, 16)
      + clip(momentum_60 * 55, -10, 10)
      + clip((0.35 - volatility_20) * 24, -10, 10)
      + liquidity_score * 4
      + rsi_balance_score * 8
      + clip(macd_hist * 3, -5, 5)
      + clip(drawdown * 40, -12, 0)
```
...
- 趋势强会加分，趋势弱会扣分。
- 20 日和 60 日动量为正会加分，为负会扣分。
- 年化波动率低于约 35% 会加分，过高会扣分。
- 流动性越好越加分。
- RSI 处在健康区间会加分，过热或过冷会扣分。
- MACD 柱为正会加分，为负会扣分。
- 当前回撤越深扣分越多，但回撤不会带来正向加分。
...
这些指标主要出现在 `compare` 命令和 Streamlit 的 `Compare` 页面中。
...
把每只股票在对比区间的起点统一成同一个基准值，当前默认为 100。

公式：

```text
normalized_t = close_t / first_close * 100
```

解读：

- 从 100 涨到 120：区间收益约 20%
- 从 100 跌到 85：区间亏损约 15%

用途：

不同股票价格绝对值可能差很多，例如一只股票 20 元，另一只股票 200 美元。标准化后可以放在同一张图上比较相对涨跌，而不是比较价格高低。
...
它只表示价格水平，不代表便宜或昂贵。判断估值还需要市盈率、市净率、现金流、行业和增长等基本面信息。
...
- `current_drawdown`：现在距离高点跌了多少。
- `max_drawdown`：整个样本期内最深曾经跌了多少。
...
相关性用于观察组合分散程度。多个高相关股票放在一起，表面上持仓很多，实际可能仍然暴露在同一种风险上。

注意：

相关性会随时间变化。短样本相关性不稳定，极端行情中原本低相关的资产也可能一起下跌。
...
横轴通常是 `annual_volatility`，纵轴是 `total_return`。它用一张散点图比较每只股票在收益和风险上的位置。

常见解读：

- 左上方：历史收益较高、波动较低，样本内表现较好。
- 右上方：收益较高但波动也高。
- 左下方：波动低但收益差。
- 右下方：收益差且波动高，样本内表现较弱。

项目中散点颜色使用 `max_drawdown`，点大小使用 `sharpe` 近似表达风险调整后收益。
...
这些指标主要出现在 `optimize` 命令和单票页面的 `Optimize` 标签中。
...
- `short_window`：短期均线窗口，对近期价格更敏感。
- `long_window`：长期均线窗口，更平滑，反映中期趋势。

项目只测试 `short_window < long_window` 的组合，因为短均线应该比长均线更快反应。
...
- `sharpe`：优先选择风险调整后收益较高的组合。
- `total_return`：优先选择累计收益最高的组合。
- `annual_return`：优先选择年化收益最高的组合。
- `max_drawdown`：注意这个值通常为负数，直接降序排序时，`-0.05` 会排在 `-0.20` 前面，也就是回撤较小更靠前。
- `excess_total_return`：优先选择相对买入持有基准更强的组合。
...
- 交易次数过少：样本可能不足，绩效可能不稳定。
- 交易次数过多：容易被手续费、滑点和实际成交约束侵蚀。
...
```text
MA Optimization: sharpe
```

含义：

用热力图展示不同 `short_window` 和 `long_window` 组合在某个绩效指标上的表现。

解读：

- 如果只有单个参数点表现特别好，可能是偶然拟合。
- 如果一片相邻参数区域都表现较好，参数稳定性相对更强。
- 如果换一个时间段结果完全变化，需要警惕过拟合。

### Parameter Overfitting

含义：

参数过拟合是指某组参数只是在历史样本上碰巧表现最好，但未来不一定有效。
...
这些字段主要出现在 `alerts` 命令和 Streamlit 的 `Alerts` 页面。预警用于提示“值得检查”，不是自动买卖建议。
...
- `min_score`：综合分达到该值，触发高分预警。
- `max_rsi`：RSI 高于该值，触发超买或过热预警。
- `min_rsi`：RSI 低于该值，触发超卖或弱势预警。
- `max_drawdown`：当前回撤低于该值，触发回撤预警。例如 `-0.10` 表示回撤达到 10%。
- `min_momentum_20_pct`：20 日动量低于该值，触发弱动量预警。
- `include_bearish`：是否把 `bearish` 趋势也作为预警。
- `include_all`：是否保留没有触发预警的股票，方便完整查看股票池。
...
- `0`：没有触发当前规则。
- `1`：有一个维度值得关注。
- `2` 或更多：多个维度同时异常，需要优先检查。

注意：

触发数量多不一定代表要卖出或买入。比如高分和 RSI 过热可能同时出现，含义是“趋势强但短期偏热”。
...
- `score>=80`：综合分高，说明样本内趋势、动量、风险等综合表现较强。
- `rsi_overbought>=75`：RSI 偏高，说明短期可能过热。
- `rsi_oversold<=30`：RSI 偏低，说明短期可能超卖，也可能是弱势延续。
- `drawdown<=-10%`：当前离阶段高点跌幅较深。
- `weak_momentum<=-5%`：近 20 日动量较弱。
- `bearish_trend`：趋势分偏空。
- `none`：没有触发预警，通常只有开启 `include_all` 时会显示。
